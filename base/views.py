...
# Volunteer and Project views
class AddVolunteerView(CreateView):
    model = Volunteer
    form_class = VolunteerForm
    template_name = 'base/add_volunteer.html'

    def form_valid(self, form):
        form.instance.user = self.request.user
        return super().form_valid(form)

class MatchVolunteerView(ListView):
    model = VolunteerProjectMatch
    template_name = 'base/match_volunteers.html'

    def get_context_data(self, *args, **kwargs):
        context = super(MatchVolunteerView, self).get_context_data(*args, **kwargs)
        volunteer = Volunteer.objects.get(user=self.request.user)
        matched_projects = Post.objects.filter(interests__in=volunteer.interests.all())
